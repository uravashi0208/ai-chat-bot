...
export default function ChatWindow({ conversation, onBack }) {
  ...
  const [msgInfo, setMsgInfo] = useState(null); // for message info modal
  ...
  const loadMessages = useCallback(
    async (before = undefined) => {
      try {
        ...
        // Normalize: Supabase join returns sender as object { id, ... }, ensure sender_id is present
  ...
  useEffect(() => {
    ...
    const onNewMessage = ({ conversationId, message }) => {
      ...
      // Normalize: ensure sender_id is always present (Supabase join returns sender as object)
      const normalized = {
        ...message,
      ...
      setMessages((prev) => {
        ...
        // Replace optimistic (tempId) message sent by current user with the real one
        ...
        // Otherwise it's a message from someone else — just append
    ...
    // When the other person reads the conversation → mark our sent messages as "read"
    const onConversationRead = ({ conversationId, userId }) => {
      if (conversationId !== conversation.id) return;
      if (userId === user.id) return; // skip own read event
    ...
    // message:status from backend (sent → delivered → read with timestamps)
  ...
  const handleSend = useCallback(
    async ({ content, type = "text", mediaUrl, replyToId }) => {
      // Optimistic message
