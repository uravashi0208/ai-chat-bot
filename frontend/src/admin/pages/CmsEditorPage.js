...
import {
  Box, Stack, Button, TextField, Typography,
...
export default function CmsEditorPage({ type = "privacy" }) {
  ...
  useEffect(() => {
    let alive = true;
    setLoading(true);
    (isPrivacy ? adminCmsApi.getPrivacyPolicy() : adminCmsApi.getTermsConditions())
      .then((d) => {
        if (!alive) return;
        setData(d);
        setTitle(d?.title || (isPrivacy ? "Privacy Policy" : "Terms & Conditions"));
    ...
    <Box>
      <PageHeader
        ...
        actions={
          <Button
            variant="contained"
            size="small"
            startIcon={saving ? <CircularProgress size={14} color="inherit" /> : <SaveIcon sx={{ fontSize: 15 }} />}
            onClick={handleSave}
            disabled={saving || loading}
            sx={{ borderRadius: "8px", fontWeight: 600, textTransform: "none", bgcolor: "#111827", "&:hover": { bgcolor: "#374151" }, boxShadow: "none" }}
          >
      ...
      {data?.updated_at && (
        <Typography variant="caption" sx={{ color: "#9ca3af", mb: 2, display: "block" }}>
          Last updated: {format(new Date(data.updated_at), "dd MMM yyyy, HH:mm")}
        </Typography>
      )}
      ...
      {/* Live preview */}
      {!loading && content && (
        <SectionCard title="Preview" subtitle="Rendered output" sx={{ mt: 2.5 }}>
          <Box
            sx={{
              p: 2, borderRadius: "8px", bgcolor: "#fafafa", border: "1px solid #f3f4f6",
