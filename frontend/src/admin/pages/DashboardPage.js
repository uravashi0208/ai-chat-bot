...
export default function DashboardPage() {
  ...
  const hasFetchedDash = useRef(false);
  useEffect(() => {
    // Guard against React StrictMode double-invoking this effect in development
    if (hasFetchedDash.current) return;
    hasFetchedDash.current = true;

    let alive = true;
    (async () => {
      try {
        const [users, emojis, themes, wallpapers, feedback, contact, faqs] =
        ...
        console.log("users :", users.value.total, alive);
              ...
              >
                <Box
                  sx={{
                    ...
                    borderRadius: "9px",
                    bgcolor: (t) =>
                      alpha(t.palette[l.color]?.main || "#6366f1", 0.1),
                    display: "flex",
                    alignItems: "center",
